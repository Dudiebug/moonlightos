...
releases by `scripts/fetch-apps.sh` using `build/applications.lock`. Their
license texts and pinned upstream source snapshots are included under
`/usr/share/doc/moonlightos/`; see `SOURCE_CODE.md`. MoonlightOS does
...
Firefox ESR and Chromium are optional owner-installed software. They are not
included in or distributed with the ISO; the Settings screen can install the
selected package directly from Debian's signed repositories.
